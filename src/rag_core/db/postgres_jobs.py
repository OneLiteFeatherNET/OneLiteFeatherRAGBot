...
class PostgresJobRepository(JobRepository):
    ...
    async def list(self, limit: int = 20, status: Optional[str] = None) -> List[Job]:
        ...
        try:
            await self.ensure()
            base_cols = "id, type, payload, status, attempts, error, created_at, started_at, finished_at, progress_done, progress_total, progress_note"
            if status:
                rows = await conn.fetch(
                    f"SELECT {base_cols} FROM {self.table} WHERE status=$1 ORDER BY id DESC LIMIT $2",
                    status,
                    limit,
                )
            else:
                rows = await conn.fetch(
                    f"SELECT {base_cols} FROM {self.table} ORDER BY id DESC LIMIT $1",
                    limit,
                )
            out: List[Job] = []
            for r in rows:
